            ...
            try
            {
                ...
                if (subKey != null)
                {
                    switch (subKey.GetValue(string.Empty).ToString().ToUpperInvariant())
                    {
                        ...
                        default:
                            throw new OCWordConfiguration("Could not determine PowerPoint version");
                    }
                }
                else
                    throw new OCWordConfiguration("Could not find registry key PowerPoint.Application\\CurVer");
            }
            catch (Exception exception)
            {
                throw new OCWordConfiguration("Could not read registry to check PowerPoint version", exception);
            }
        }
        #endregion
